...
public class VentanaRegistroPacientes {
    ...
    public VentanaRegistroPacientes(JFrame parent, VentanaDetallesOrdenes ventanaDetalles) {
        ...
        gbc.gridx = 3;
        nacionalidadCombo = new JComboBox<>(new String[]{"V", "E", "A"});
    ...
    private static int calcularEdadDesdeTexto(String fechaTexto) {
        try {
            if (fechaTexto == null || fechaTexto.trim().isEmpty()) return -1;
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
            LocalDate nacimiento = LocalDate.parse(fechaTexto.trim(), formatter);
    ...
    private void guardarPaciente() {
        ...
        if (nacionalidad == null) nacionalidad = "V"; // Por defecto V
        
        // RN sin cédula: generar A+codigo
        String cedulaCalc = cedulaField.getText().trim();
        if (cedulaCalc.isEmpty() && rnCheck.isSelected()) {
            try {
                nacionalidadCombo.setSelectedItem("A");
                nacionalidad = "A";
            } catch (Exception ignored) {}
            String codigo = codigoField.getText().trim();
            if (codigo.isEmpty()) codigo = proponerSiguienteCodigo();
            String digits = codigo.replaceAll("[^0-9]", "");
            if (digits.isEmpty()) digits = "0000";
            cedulaCalc = "A" + digits;
            cedulaField.setText(cedulaCalc);
            datos.put("cedula", cedulaCalc);
        } else {
            // Asegurar que la cédula tenga la nacionalidad al inicio
            String cedulaSinNacionalidad = cedulaCalc;
            // Si ya empieza con V, E o A, removerlo para reconstruir
            if (cedulaSinNacionalidad.length() > 0 && 
                (cedulaSinNacionalidad.startsWith("V") || 
                 cedulaSinNacionalidad.startsWith("E") || 
                 cedulaSinNacionalidad.startsWith("A"))) {
                cedulaSinNacionalidad = cedulaSinNacionalidad.substring(1);
            }
            // Reconstruir con la nacionalidad seleccionada
            cedulaCalc = nacionalidad + cedulaSinNacionalidad;
            cedulaField.setText(cedulaCalc);
            datos.put("cedula", cedulaCalc);
        }

        // Asegurar edad calculada desde fecha si no está puesta manualmente
        int edadCalculada = safeParseInt(datos.getOrDefault("edad", "0"));
        if (edadCalculada <= 0) {
            String fechaNac = datos.getOrDefault("fechaNac", "");
            edadCalculada = calcularEdadDesdeTexto(fechaNac);
            if (edadCalculada < 0) edadCalculada = 0;
            datos.put("edad", String.valueOf(edadCalculada));
            edadField.setText(String.valueOf(edadCalculada));
        }
        // �� Obtener fecha de nacimiento del formulario
        LocalDate fechaNacimiento = null;
        String fechaNacTexto = fechaNacField.getText().trim();
        ...
        try {
            ...
            Paciente p = new Paciente(
                    cedulaCalc.isEmpty() ? cedula : cedulaCalc,
        ...
        // Notificar a VentanaDetallesOrdenes con el paciente completo
        String cedulaFinal = cedulaCalc.isEmpty() ? cedula : cedulaCalc;
        
        // Crear el objeto Paciente con los datos del formulario para pasarlo directamente
        String sexo = datos.getOrDefault("sexo", "");
        // Convertir "Masculino"/"Femenino" a "M"/"F"
        if (sexo.equalsIgnoreCase("Masculino")) sexo = "M";
        else if (sexo.equalsIgnoreCase("Femenino")) sexo = "F";
        
        Paciente pacienteParaCargar = new Paciente(
                cedulaFinal,
                datos.getOrDefault("nombres", ""),
                datos.getOrDefault("apellidos", ""),
                edadCalculada,
        ...
        // Notificar a VentanaDetallesOrdenes con el paciente completo (sin necesidad de buscar en BD)
        if (ventanaDetalles != null && !cedulaFinal.isEmpty()) {
            // Pasar el paciente completo directamente desde los datos del formulario
            ventanaDetalles.cargarPacienteCompleto(pacienteParaCargar);
        }
        
        // Recargar la lista de pacientes si hay una referencia a VentanaPacientes
        if (ventanaPacientes != null) {
            ventanaPacientes.recargarPacientes();
        }
    ...
    private void actualizarPaciente() {
        ...
        Map<String, String> datos = obtenerDatosFormulario();
        // Recalcular edad si está vacía y hay fecha de nacimiento
        int edadCalculada = safeParseInt(datos.getOrDefault("edad", "0"));
        if (edadCalculada <= 0) {
            edadCalculada = calcularEdadDesdeTexto(datos.getOrDefault("fechaNac", ""));
            if (edadCalculada < 0) edadCalculada = 0;
            datos.put("edad", String.valueOf(edadCalculada));
        }
    ...
    public VentanaRegistroPacientes(Paciente paciente, VentanaDetallesOrdenes ventanaDetalles) {
        ...
        if (paciente != null) {
            ...
            // Extraer y seleccionar la nacionalidad de la cédula
            if (cedula != null && !cedula.trim().isEmpty()) {
                String cedulaUpper = cedula.trim().toUpperCase();
                if (cedulaUpper.startsWith("V")) {
                    nacionalidadCombo.setSelectedItem("V");
                } else if (cedulaUpper.startsWith("E")) {
                    nacionalidadCombo.setSelectedItem("E");
                } else if (cedulaUpper.startsWith("A")) {
                    nacionalidadCombo.setSelectedItem("A");
                } else {
                    nacionalidadCombo.setSelectedItem("V"); // Por defecto
                }
            }
            ...
            if (paciente.getSexo() != null && !paciente.getSexo().trim().isEmpty()) {
                String sexo = paciente.getSexo().trim().toUpperCase();
                if (sexo.equals("M") || sexo.equalsIgnoreCase("Masculino")) {
                    sexoCombo.setSelectedItem("Masculino");
                } else if (sexo.equals("F") || sexo.equalsIgnoreCase("Femenino")) {
                    sexoCombo.setSelectedItem("Femenino");
                }
            }
            if (paciente != null && paciente.getFechaNacimiento() != null) {
                fechaNacField.setText(paciente.getFechaNacimiento()
                    .format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
                calcularEdad(); // Recalcular edad automáticamente
            }
        }
    }
    
    // Método para establecer la referencia a VentanaPacientes
    public void setVentanaPacientes(VentanaPacientes ventanaPacientes) {
